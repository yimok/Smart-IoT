    ...
    if((tick_count % 2) == 0) {
        if (tas_state == 'upload') {
           // var con = 'TAS' + count++ + ',' + '55.2';
			...
			var firsttemp = cowtemp ;
            
			if(Math.abs(firsttemp - secondtemp) > 0.1 )
			{
					var con = 'TEMP/WLK:' +  cowtemp + ',' + extemp +','+wlk;
...
function setExTemp()     
{
	temp3 = Math.floor(Math.random() *1) + 22;
	temp4 = Math.floor(Math.random() *80) + 11;
	extemp = temp3 + "." + temp4
...
function setActive()     
{
	wlk = Math.floor(Math.random() *3) + 9;
}

//랜덤값을 얻는 온도 함수 .
function setCowTemp()     
{
	temp1 = Math.floor(Math.random() *1) + 37;
	temp2 = Math.floor(Math.random() *80) + 11;
	cowtemp = temp1 + "." + temp2
}
